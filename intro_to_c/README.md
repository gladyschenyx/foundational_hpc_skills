...
The `int main()` is a declaration of a function called main, which is where execution of the program begins.  `main` is included in all C programs. The “int” indicates that the function will return an integer value.
...
`return 0` is a a return value “returned” to the run-time environment. Typically, a value of 0 indicates a normal/successful exit.

To compile the C program, use the gcc compiler to compile the C code into executable:
```
$ gcc simple.c
```
